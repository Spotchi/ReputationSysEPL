...
However, we want to have a unique weight for each judge. In order to find that new rating, we need to account for the consistency of a judge on all characteristics. The model we propose lowers the weight of a judge as the judge's rating recedes from the reputation for each relevant object and characteristic.
\begin{table}
\centering
\begin{tabular}{|c|c|}
\hline 
...
This last vector can be seen as the distance of the ratings of the judge $i$ for object $j$ from the reputation of the object $j$.
...
\end{itemize}
Let us rewrite the iteration
\begin{eqnarray}
R^t_{jk} & = & \frac{\sum_{i}X_{ijk}w^{t-1}_{i}}{\sum_i w^{t-1}_{i}}\label{eq:rep}\\
w^t_{i}(X,r,C) = 1 -k div^t_i \label{eq:w8}
\end{eqnarray}
with $k= \frac{1}{(\log(2\pi \sigma^2))}$ and $div^t_i =  \frac{1}{MK}\sum_{j} ((d^{ij})^t)^T (d^{ij})^t$ and $ (d^{ij})^t_k = X_{ijk}-R^{t-1}_{jk}$\\
...
\label{eq:poly}
\end{lemma}
\todo[inline]{Mettre la correction du lemme?}
...
\todo[inline]{Idée de proposition : quand on change $k$, l'ordre des poids ne changent pas. Aussi, la méthode est limitée par le fait qu'on peut mettre un énorme tricheur qui va fixer la valeur du $k$ possible et puis ensuite rajouter un tricheur un peu plus subtil qui va avoir un poids correct}
...
\subsection{Preprocessing}
Our experiment concerns a data set of 12773 hotels evaluated by 1169410 users on 10 characteristics and raging from 1 to 5. But we select a subset of this data set. Indeed, the hotels that have been evaluated by 1 user does not interest us, as users who would have voted for a hotel. This allows us to reduce the data set to a smaller and more easily usable. To further reduce the data we have selected the users who voted for at least 4 hotels. The subset have 305 hotels evaluated by 20024 users on 9 characteristics. We notice that the dataset is extremely sparse.

\subsection{Analyse results} 

After iterations we observe a distribution of weights illustrated at figure (\ref{fig:hotel:initial_weights}). We note that most users are consistent and thus the weights are high. Unfortunately, we do not detect a specific behavior.
...
In order to simulate the robustness of our algorithm, two types of behavior are analyzed in the sequel: first, raters that give random evaluations, and second, spammers that try to improve the reputation of their preferred hotel.

\subsubsection*{Robustness against random raters}
We added to the original data set 2000 raters evaluating randomly some hotels. In that manner, 10\% of the raters give random evaluations. The figure (\ref{fig:hotel:random_cheaters}) illustrate the distribution of weights after iteration with and without spammers.
...
Here, spammers are perfectly benchmarks and their weight is quite low.\\

We now added to the original data set one spammer per hotel giving always 0 except for their own hotel, which they rated 5. The figure (\ref{fig:hotel:not_random_each_hotel}) illustrate the distribution of weights after iteration with and without spammers.
