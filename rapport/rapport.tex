...
The figure (\ref{fig:howto}) show you a case where we have two teams, and three different ways to evaluated them. The goal of the cheater is helping the first team to win. That is why we only observe the score obtained by the first team called "Advantaged Team". We present three cases.
\begin{enumerate}
    ...
    \item The iteration method. In this case we see that if the cheater wants the note of the favored team is maximum, then it should not give them the maximum score!
\end{enumerate}
We can conclude that the iterative method does not allow the cheater know what to do.

\begin{figure}[h!]
\centering
\includegraphics[width = \textwidth]{cheaters/howTo.eps}
