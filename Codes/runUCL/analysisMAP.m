...
[mat,A,teachers,students,caracs] = createTensor();
...
for i = 1 : length(k)
    ...
    filename = sprintf('distribLeastRelK%fc1.eps',k(i));
    ...
    filename = sprintf('distribLeastRelK%fc2.eps',k(i));
    print(strcat(images_dir,filename),'-depsc');
    close(fig);
    ...
    ratings1 = mat(A(:,:,1)==1);
    hist(ratings1(:)');
    filename = sprintf('ratingsHistK%fc1.eps',k(i));
    print(strcat(images_dir,filename),'-depsc');
    close(fig);
    ...
    ratings2 = ratings2(:)';
    hist(ratings2(ind));
    filename = sprintf('ratingsHistK%fc2.eps',k(i));
    print(strcat(images_dir,filename),'-depsc');
    close(fig);
    
    fig = createFigure('Histogram of reputations for characteristic 1');
    hist(R(:,1));
    filename = sprintf('reputationHistK%fc1.eps',k(i));
    print(strcat(images_dir,filename),'-depsc');
    close(fig);
    
    fig = createFigure('Histogram of reputations for characteristic 2');
    hist(R(:,1));
    filename = sprintf('reputationHistK%fc2.eps',k(i));
    print(strcat(images_dir,filename),'-depsc');
    close(fig);
    
    fig = createFigure('Histogram of d after iteration');
    hist(d);
    filename = sprintf('dHistK%f.eps',k(i));
    print(strcat(images_dir,filename),'-depsc');
    close(fig);
    
end

images_dir = '../../rapport/images/preprocess/';
pmat = preprocess(mat,A);
